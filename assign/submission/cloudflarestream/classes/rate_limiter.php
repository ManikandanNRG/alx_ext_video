...
/**
 * Rate limiting functionality for Cloudflare Stream plugin.
 *
 ...
 * and playback token requests using Moodle's cache API.
 */
class rate_limiter {
    ...
    /** @var int Default rate limit for playback token requests (per user per hour) */
    ...
    public function __construct() {
        $this->cache = \cache::make('assignsubmission_cloudflarestream', self::CACHE_AREA);
    }
    ...
    public function check_upload_rate_limit($userid, $assignmentid) {
        $limit = get_config('assignsubmission_cloudflarestream', 'upload_rate_limit') ?: self::DEFAULT_UPLOAD_LIMIT;
    ...
    /**
     * Check if playback token request is allowed for a user.
     *
     * @param int $userid User ID
     * @param string $videouid Video UID (for additional context)
     * @throws rate_limit_exception If rate limit is exceeded
     */
    public function check_playback_rate_limit($userid, $videouid) {
        $limit = get_config('assignsubmission_cloudflarestream', 'playback_rate_limit') ?: self::DEFAULT_PLAYBACK_LIMIT;
     ...
     * @param int $userid User ID
     * @param int|string $context Assignment ID for upload, video UID for playback
     */
    ...
    public function is_rate_limit_exempt($userid) {
        ...
        if (has_capability('assignsubmission/cloudflarestream:bypassratelimit', $systemcontext, $userid)) {
            return true;
        }
